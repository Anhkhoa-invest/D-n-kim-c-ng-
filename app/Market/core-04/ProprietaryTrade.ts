...
export class MarketDepth {
  ...
  getStrongestBid(): MarketDepthRecord | null {
    ...
    return this.records.reduce((best, current) =>
      current.totalBidVolume > best.totalBidVolume ? current : best
    );
  }
 ...
 * Kim Cương OS
 * Market Core-04
 * Proprietary Trade Analyzer
 */

export interface ProprietaryTradeRecord {
  symbol: string;
  buyValue: number;
  sellValue: number;
  netValue: number;
...
export class ProprietaryTrade {

  constructor(
    private records: ProprietaryTradeRecord[] = []
  ) {}

  add(record: ProprietaryTradeRecord): void {
    this.records.push(record);
  }

  getAll(): ProprietaryTradeRecord[] {
    return this.records;
  }
  ...
  getTopBuy(): ProprietaryTradeRecord | null {
    if (this.records.length === 0) return null;

    return this.records.reduce((best, current) =>
      current.buyValue > best.buyValue ? current : best
    );
  }
